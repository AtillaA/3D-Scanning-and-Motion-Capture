...
public:
	...
	double Eval(const Eigen::Vector3d& _x) {
		...
		double temp = pow(_x[0] - m_center[0], 2) + pow(_x[1] - m_center[1], 2) + pow(_x[2] - m_center[2], 2) + pow(m_radius, 2) - pow(m_a, 2);
		return pow(temp, 2) - 4.0 * pow(m_radius, 2) * ( pow(_x[0] - m_center[0], 2) * pow( _x[1] - m_center[1], 2) );
		// ------------------------------------------------------------------------------------------------------------------------------------------
	}
...
{
public:
	Hoppe(const std::string& filenamePC) { m_pointcloud.ReadFromFile(filenamePC); }
	...
	double Eval(const Eigen::Vector3d& _x) {
		...
		double dist = (double)(interVector.dot(n));
		return dist;
		// -----------------------------------------------------------------------------------------
	}
...
private:
	...
	double EvalBasis(double x) { return x*x*x; }
