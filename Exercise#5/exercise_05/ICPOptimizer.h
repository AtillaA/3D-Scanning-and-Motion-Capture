...
public:
	...
	bool operator()(const T* const pose, T* residuals) const {
		...
		T tempP[3];  //source point after rigid transform
		T sourceP[3];

		sourceP[0] = T(m_sourcePoint.x());
		sourceP[1] = T(m_sourcePoint.y());
		sourceP[2] = T(m_sourcePoint.z());

		const T* rotation = pose;
		const T* translation = pose + 3;

		ceres::AngleAxisRotatePoint(rotation, src, tempP);

		tempP[0] += translation[0];
		tempP[1] += translation[1];
		tempP[2] += translation[2];

		T weight = ceres::sqrt(T(m_weight));

		residuals[0] = (T(m_targetPoint.x()) - p[0]) * weight;
		residuals[1] = (T(m_targetPoint.y()) - p[1]) * weight;
		residuals[2] = (T(m_targetPoint.z()) - p[2]) * weight;
		// ---------------------------------------------------------------------------------------
